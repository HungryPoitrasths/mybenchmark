...
class RunPipelineReferabilityTests(unittest.TestCase):
    ...
    def test_scene_status_v5_is_rejected_after_candidate_budget_migration(self) -> None:
    ...
    def test_scene_type_cap_does_not_limit_final_l1_types(self) -> None:
        ...
        self.assertEqual([q["question"] for q in kept if q["type"] == "direction_agent"], [f"d {idx}" for idx in range(7)])
        self.assertEqual([q["question"] for q in kept if q["type"] == "distance"], [f"m {idx}" for idx in range(6)])
        self.assertEqual([q["question"] for q in kept if q["type"] == "occlusion"], [f"o {idx}" for idx in range(7)])
    ...
    def test_load_cached_scene_questions_does_not_apply_final_type_cap(self) -> None:
        ...
        self.assertEqual([q["question"] for q in loaded], [f"occlusion {idx}" for idx in range(8)])

    def test_remaining_candidate_budgets_tracks_requested_types(self) -> None:
        budgets = run_pipeline_module._remaining_candidate_type_budgets(
            Counter(
                {
                    "occlusion": 3,
        ...
        self.assertEqual(
            budgets,
            {"distance": 1, "object_move_agent": 1, "occlusion": 2},
        )

    def test_final_filter_only_applies_object_frame_caps(self) -> None:
        questions = [
    ...
    def test_remaining_candidate_budgets_include_l2_l3_types(self) -> None:
        budgets = run_pipeline_module._remaining_candidate_type_budgets(
        ...
        self.assertEqual(
            budgets,
            {"attachment_chain": 1, "object_move_agent": 1, "occlusion": 2},
        )

    def test_remaining_candidate_budgets_return_none_when_unlimited(self) -> None:
        budgets = run_pipeline_module._remaining_candidate_type_budgets(
    ...
    def test_load_referability_cache_scene_status_allows_model_mismatch(self) -> None:
        ...
        with self.assertLogs(run_pipeline_module.logger.name, level="WARNING") as logs:
            loaded = run_pipeline_module._load_referability_cache(status_path)

        self.assertEqual(sorted(loaded["scene_status"]), [scene_a, scene_b])
        self.assertIn("generated by multiple VLM models", "\n".join(logs.output))

    def test_load_referability_cache_scene_status_rejects_schema_metadata_mismatch(self) -> None:
        case_dir = make_case_dir("cache_scene_status_schema_metadata")
        self.addCleanup(shutil.rmtree, case_dir, True)
        batch_a = case_dir / "flash_a.json"
        batch_b = case_dir / "flash_b.json"
        status_path = case_dir / "scene_status.json"
        ...
        batch_b_doc = make_referability_batch_doc(scene_id=scene_b)
        batch_b_doc["alias_config_version"] = "other-alias"
        batch_b.write_text(json.dumps(batch_b_doc, ensure_ascii=False), encoding="utf-8")
        write_referability_scene_status(
            status_path,
            {scene_a: batch_a.name, scene_b: batch_b.name},
        )

        with self.assertRaisesRegex(ValueError, "metadata mismatch for alias_config_version"):
            run_pipeline_module._load_referability_cache(status_path)
